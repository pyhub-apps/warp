...
/// Clear cache data
async fn clear_cache(cache_config: &crate::cache::CacheConfig, api: Option<String>, force: bool) -> Result<()> {
    if !force {
    ...
    if let Some(api_type) = api {
        ...
        let api_type_enum = match api_upper.as_str() {
            ...
            "EXPC" => ApiType::Expc,
            _ => {
                println!("{}", "올바른 API 타입을 지정해주세요: nlic, elis, prec, admrul, expc".red());
                return Ok(());
            }
        };
...
fn show_config(config: &Config) -> Result<()> {
    ...
    println!("  {} {}", 
        "활성화:".bold(), 
        if cache_config.enabled { "Yes".green() } else { "No".red() }
    );
    println!("  {} {} 시간", "TTL:".bold(), cache_config.ttl_seconds / 3600);
...
async fn enable_cache(config: &Config) -> Result<()> {
    ...
    let toml_content = toml::to_string_pretty(&config)
        .map_err(|e| crate::error::WarpError::Config(format!("Failed to serialize config: {}", e)))?;
