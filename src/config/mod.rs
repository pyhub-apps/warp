    ...
    pub fn get_cache_db_path(&self) -> Result<PathBuf> {
        ...
        if !cache_dir.exists() {
            std::fs::create_dir_all(&cache_dir)
                .map_err(|e| WarpError::Config(format!("Failed to create cache directory: {}", e)))?;
            ...
            #[cfg(unix)]
            {
                ...
                std::fs::set_permissions(&cache_dir, permissions)
                    .map_err(|e| WarpError::Config(format!("Failed to set cache directory permissions: {}", e)))?;
            }
        }
        ...
        if db_path.exists() {
            #[cfg(unix)]
            {
                ...
                std::fs::set_permissions(&db_path, permissions)
                    .map_err(|e| WarpError::Config(format!("Failed to set cache database permissions: {}", e)))?;
            }
        }
...
impl Config {
    ...
    pub fn set(&mut self, key: &str, value: &str) -> Result<()> {
        match key {
            ...
            "cache.enabled" => {
                self.cache.enabled = value.parse::<bool>()
                    .map_err(|_| WarpError::Config(format!("Invalid boolean value: {}", value)))?;
            }
            "cache.ttl_seconds" => {
                self.cache.ttl_seconds = value.parse::<u64>()
                    .map_err(|_| WarpError::Config(format!("Invalid TTL seconds value: {}", value)))?;
            }
            "cache.max_size_mb" => {
                self.cache.max_size_mb = value.parse::<u64>()
                    .map_err(|_| WarpError::Config(format!("Invalid cache size value: {}", value)))?;
            }
            "cache.cache_dir" => {
...
"#;
        ...
        assert_eq!(config.cache.cache_dir, Some(PathBuf::from("/custom/cache/path")));
    }
