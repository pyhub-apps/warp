...
impl SqliteStorage {
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            ...
            if is_new_file {
                #[cfg(unix)]
                {
                    ...
                    std::fs::set_permissions(&db_path_buf, permissions)
                        .map_err(|e| WarpError::Other(format!("Failed to set cache database permissions: {}", e)))?;
                }
            }
            ...
            Ok(())
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn database initialization: {}", e)))?;
    ...
    pub async fn get(&self, key: &str) -> Result<Option<CacheEntry>> {
        ...
        tokio::task::spawn_blocking(move || -> Result<Option<CacheEntry>> {
            ...
            let entry = stmt.query_row(params![&key_owned], |row| {
                let api_type_str: String = row.get(4)?;
                let api_type = ApiType::from_str(&api_type_str)
            ...
            Ok(entry)
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn get operation: {}", e)))?
    }
    ...
    pub async fn put(&mut self, entry: CacheEntry) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            ...
            Ok(())
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn put operation: {}", e)))?
    }
    ...
    pub async fn remove(&mut self, key: &str) -> Result<bool> {
        ...
        tokio::task::spawn_blocking(move || -> Result<bool> {
            ...
            Ok(affected > 0)
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn remove operation: {}", e)))?
    }
    ...
    pub async fn clear(&mut self) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            ...
            Ok(())
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn clear operation: {}", e)))?
    }
    ...
    pub async fn clear_by_api_type(&mut self, api_type: ApiType) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            ...
            Ok(())
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn clear_by_api_type operation: {}", e)))?
    }
    ...
    pub async fn count_entries(&self) -> Result<u64> {
        ...
        tokio::task::spawn_blocking(move || -> Result<u64> {
            ...
            Ok(count as u64)
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn count_entries operation: {}", e)))?
    }
    ...
    pub async fn get_total_size(&self) -> Result<u64> {
        ...
        tokio::task::spawn_blocking(move || -> Result<u64> {
            ...
            Ok(size as u64)
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn get_total_size operation: {}", e)))?
    }
    ...
    pub async fn count_expired_entries(&self) -> Result<u64> {
        ...
        tokio::task::spawn_blocking(move || -> Result<u64> {
            ...
            Ok(count as u64)
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn count_expired_entries operation: {}", e)))?
    }
    ...
    pub async fn cleanup_expired(&mut self) -> Result<u64> {
        ...
        tokio::task::spawn_blocking(move || -> Result<u64> {
            ...
            let affected = conn.execute(
                ...
                params![now],
            ).map_err(|e| WarpError::Other(format!("Failed to cleanup expired entries: {}", e)))?;

            Ok(affected as u64)
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn cleanup_expired operation: {}", e)))?
    }
    ...
    pub async fn evict_lru(&mut self, space_to_free: u64) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            ...
            for row in rows {
                entries.push(row.map_err(|e| WarpError::Other(format!("Failed to process LRU row: {}", e)))?);
            }
            ...
            Ok(())
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn evict_lru operation: {}", e)))?
    }
    ...
    pub async fn get_entries_by_api_type(&self, api_type: ApiType) -> Result<Vec<CacheEntry>> {
        ...
        tokio::task::spawn_blocking(move || -> Result<Vec<CacheEntry>> {
            ...
            "#,
        ).map_err(|e| WarpError::Other(format!("Failed to prepare API type query: {}", e)))?;

        let mut entries = Vec::new();
        let rows = stmt.query_map(params![api_type.as_str()], |row| {
            let api_type_str: String = row.get(4)?;
            let api_type = ApiType::from_str(&api_type_str)
                .ok_or_else(|| rusqlite::Error::InvalidColumnType(
                    4, 
        ...
        }).map_err(|e| WarpError::Other(format!("Failed to execute API type query: {}", e)))?;

        for row in rows {
            entries.push(row.map_err(|e| WarpError::Other(format!("Failed to process entry row: {}", e)))?);
        }

            Ok(entries)
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn get_entries_by_api_type operation: {}", e)))?
    }
    ...
    pub async fn optimize(&mut self) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> Result<()> {
            ...
            Ok(())
        }).await.map_err(|e| WarpError::Other(format!("Failed to spawn optimize operation: {}", e)))?
    }
}
