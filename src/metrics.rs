...
impl PerformanceMetrics {
    ...
    pub fn get_stats(&self) -> MetricsSnapshot {
        ...
        let api_stats = self.api_metrics.read()
            .unwrap()
            .iter()
    ...
    fn update_min_time(&self, duration_ms: u64) {
        ...
        if current == 0 || duration_ms < current {
            self.min_request_time_ms.store(duration_ms, Ordering::Relaxed);
        }
    }
    ...
    fn update_max_time(&self, duration_ms: u64) {
        ...
        if duration_ms > current {
            self.max_request_time_ms.store(duration_ms, Ordering::Relaxed);
        }
    }
    ...
    pub fn format(&self) -> String {
        ...
        if !self.api_stats.is_empty() {
            ...
            for (api, stats) in &self.api_stats {
                ...
                output.push_str(&format!("  {}: {} req, {:.1}% success, {}ms avg\n", 
                    api, stats.requests, 
                    if stats.requests > 0 { (stats.successes as f64 / stats.requests as f64) * 100.0 } else { 0.0 },
                    avg_time
                ));
            }
        }
...
impl OperationTimer {
    ...
    pub fn finish_success(self) {
        ...
        self.metrics.record_request_success(&self.api_name, duration);
    }
    
    /// Finish timing and record failure
    pub fn finish_failure(self) {
        let duration = self.start.elapsed();
        self.metrics.record_request_failure(&self.api_name, duration);
    }
}
